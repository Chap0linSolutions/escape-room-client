...
export class GeniusLogic {
    ...
    private nextState(){
        switch(this.state){
            case states.STARTING: return states.SHOWING;
            case states.SHOWING: 
                return (this.index < this.currentLevel.length - 1)
                ? states.RESTING
                : states.LISTENING;
            case states.RESTING: return states.SHOWING;
            case states.LISTENING: return states.COMPARING;
            case states.CLICK: return states.LISTENING;
        }
    }
    ...
    compare(){
        if((!this.currentLevel.length) || (this.playerGuess.length !== this.currentLevel.length)) return false;
    ...
    registerClick(buttonName: string){                  //each time the player clicks a button, 
    ...
    startNewLevel(newLength?: number){
        this.resetLevel();
        const nextLevelLength = (newLength)? newLength : (this.currentLevel.length + 1);
