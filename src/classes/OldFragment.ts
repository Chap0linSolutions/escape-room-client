...
export class Fragment {
  ...
  constructor({
    ...
    this.interactText = new FloatingText({
      text: 'interagir',
      iconSprite: cursorKey,
    });
    this.items = items;
  ...
  private setPosition(width: number, height: number) {
    ...
    this.items.forEach((item) => {
      //no primeiro render os items ainda estarão em posição relativa ao canvas como um todo, e não ao fragmento.
  ...
  interact(clickCoords: coordinate) {
    ...
    if (!this.interactions || !clickCoords)
  ...
  private drawBackground(
    ...
    this.leaveText.render(
      canvas,
      { x: this.position.x, y: this.position.y - margin - 40 },
      true
    );
    this.interactText.render(
      canvas,
      { x: this.position.x, y: this.position.y - margin - 10 },
      true
    );
  }
  ...
  private drawHitboxes(canvas: CanvasRenderingContext2D) {
    if (this.sprite.getQuad()[0] === 0) {
      this.interactions.open.forEach((o) =>
        renderHitbox(canvas, this.getAbsoluteCoords(o.coordinate), o.radius)
      );
    } else {
      this.interactions.close.forEach((o) =>
        renderHitbox(
          canvas,
          this.getAbsoluteCoords(o.coordinate),
          o.radius,
          '#800080'
        )
      );
